...
The implementation is intentionally not yet the complete monitor. ISA coverage, the full R2 generator, floating point, commands, debugger, persistence, and terminal UI remain backlog work. Normative source identities are recorded in `norms/manifest.toml`; the current bootstrap uses only the pinned `rv_i` extract while the full generator is being built.
