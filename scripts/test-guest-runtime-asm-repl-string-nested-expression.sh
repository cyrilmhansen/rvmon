...
printf '%s\n' \
  'LET TEXT$="HAMMURABI"' \
  '10 PRINT LEFT$(RIGHT$("0"+TEXT$,8),4)' \
  '20 PRINT MID$(LEFT$(TEXT$+"X",8),2,4)' \
  '30 LET OUT$=LEFT$(RIGHT$(TEXT$+"X",8),4)' \
  '40 PRINT OUT$' \
  '50 END' \
...
for expected in 'AMMU' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing nested string-expression result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'ERR' "$output_file" || grep -aFq -- 'mcause=' "$output_file"; then
    cat "$output_file"
    printf 'unexpected nested string-expression diagnostic or target fault\n' >&2
    exit 1
fi
printf 'guest assembly REPL nested string expression QEMU test passed\n'
