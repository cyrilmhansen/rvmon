...
for expected in '3.000000' '1.414213' '0.000000' '4.000000' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing SQR result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'unexpected SQR diagnostic\n' >&2
    exit 1
fi
printf 'guest assembly REPL SQR QEMU test passed\n'
