...
for expected in 'EXIT-OK' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing EXIT output: %s\n' "$expected" >&2
        exit 1
    fi
done
for forbidden in 'ERR'; do
    if grep -aFq -- "$forbidden" "$output_file"; then
        cat "$output_file"
        printf 'unexpected EXIT diagnostic: %s\n' "$forbidden" >&2
        exit 1
    fi
done
printf 'guest assembly REPL EXIT QEMU test passed\n'
