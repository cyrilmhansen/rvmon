...
for expected in '0.000000' '255.000000' '6699.000000' '13.000000' '222.000000' '3562.000000' '57005.000000' 'DEAD!' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing DEC result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'mcause=' "$output_file"; then
    cat "$output_file"
    printf 'unexpected DEC target fault\n' >&2
    exit 1
fi
printf 'guest assembly REPL DEC QEMU test passed\n'
