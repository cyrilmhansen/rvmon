...
for expected in \
    'trap: breakpoint' \
    'f1=0xc002000000000000' \
    'f2=0x0000000000000000' \
    'f3=0xc002000000000000' \
    '-2.250000' \
    '0x0000000082000400: 00 00 00 00 00 00 02 c0'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing integrated assembly negative fraction output: %s\n' "$expected" >&2
        exit 1
    fi
done

printf 'guest assembly REPL negative fraction QEMU test passed\n'
