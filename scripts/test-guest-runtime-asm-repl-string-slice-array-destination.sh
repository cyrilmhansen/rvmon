...
printf '%s\n' \
  ...
  'LET A$(2)=MID$(A$(0),2,4)' \
  'DIM LONGARRAY$(2)' \
  'LET LONGARRAY$(0)=LEFT$(A$(0),4)' \
  '10 PRINT A$(1)' \
  '20 PRINT A$(2)' \
  '30 PRINT LONGARRAY$(0)' \
  '40 END' \
...
for expected in 'RABI' 'AMMU' 'HAMM' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing array-destination slice result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'unexpected array-destination slice diagnostic\n' >&2
    exit 1
fi
printf 'guest assembly REPL string slice array-destination QEMU test passed\n'
