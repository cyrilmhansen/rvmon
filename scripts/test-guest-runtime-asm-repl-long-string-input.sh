...
sleep 0.2
printf '10 INPUT LONGSTRINGVALUE$\n20 PRINT LONGSTRINGVALUE$\n30 END\nRUN\n' >&3
sleep 0.6
printf 'TARGET INPUT\n' >&3
sleep 0.8
printf 'q\n' >&3
...
for expected in 'TARGET INPUT' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing long string INPUT output: %s\n' "$expected" >&2
        exit 1
    fi
done
printf 'guest assembly REPL long-string INPUT QEMU test passed\n'
