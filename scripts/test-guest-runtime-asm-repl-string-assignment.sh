...
printf '%s\n' \
  'LET TEXT$="HAMMURABI"' \
  'LET PREFIX$=LEFT$(TEXT$,4)' \
  '10 PRINT PREFIX$' \
  '20 LET PROGRAM$=LEFT$(TEXT$,5)' \
  '30 PRINT PROGRAM$' \
  '40 LET SAME$=LEFT$(TEXT$,99)' \
  '50 PRINT SAME$' \
  '60 END' \
...
for expected in 'HAMM' 'HAMMU' 'HAMMURABI' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing string-assignment result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'unexpected string-assignment diagnostic\n' >&2
    exit 1
fi
printf 'guest assembly REPL LEFT$ assignment QEMU test passed\n'
