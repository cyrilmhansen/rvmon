...
sleep 0.3
printf '%s\n' \
  '10 PRINT TAN(1.5707963267948966)' \
  '20 END' \
  'RUN' >&3
sleep 0.8
printf '%s\n' 'q' >&3
...
if ! grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'missing tangent pole diagnostic\n' >&2
    exit 1
fi
if ! grep -aFq -- 'trap: breakpoint' "$output_file"; then
    cat "$output_file"
    printf 'missing clean stop after tangent error\n' >&2
    exit 1
fi
printf 'guest assembly REPL TAN error QEMU test passed\n'
