...
for expected in \
    'trap: breakpoint' \
    'f1=0x4029000000000000' \
    'f2=0x400c000000000000' \
    'f3=0x4030000000000000' \
    '0x0000000082000400: 00 00 00 00 00 00 30 40'; do
    if ! grep -aFq "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing integrated assembly multi-digit output: %s\n' "$expected" >&2
        exit 1
    fi
done

printf 'guest assembly REPL multi-digit QEMU test passed\n'
