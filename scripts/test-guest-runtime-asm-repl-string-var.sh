...
for expected in 'FROM PROGRAM' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing integrated assembly string variable output: %s\n' "$expected" >&2
        exit 1
    fi
done
printf 'guest assembly REPL string variable QEMU test passed\n'
