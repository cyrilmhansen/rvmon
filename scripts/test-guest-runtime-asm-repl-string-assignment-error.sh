...
if (( error_count < 3 )); then
    cat "$output_file"
    printf 'expected three string-assignment diagnostics, got %s\n' "$error_count" >&2
    exit 1
fi
if ! grep -aFq -- 'trap: breakpoint' "$output_file"; then
    cat "$output_file"
    printf 'missing clean stop after string-assignment errors\n' >&2
    exit 1
fi
printf 'guest assembly REPL LEFT$ assignment error QEMU test passed\n'
