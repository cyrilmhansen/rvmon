...
printf 'DIM LONGGRID$(1,2)\nI=1\nJ=2\nLONGGRID$(1,2)="GRID-TARGET"\n10 PRINT LONGGRID$(I+0,J)\n20 PRINT LONGGRID$(0,1)\n30 END\nRUN\n' >&3
sleep 1.0
printf 'q\n' >&3
exec 3>&-
sleep 0.3
...
for expected in 'GRID-TARGET' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing long string-array 2d output: %s\n' "$expected" >&2
        exit 1
    fi
done
