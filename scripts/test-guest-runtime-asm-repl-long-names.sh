...
sleep 0.2
printf 'LONGVARIABLE16=40\n10 LONGVARIABLE16=longvariable16+2\n20 PRINT "LONG",longvariable16\n30 END\nRUN\n' >&3
sleep 0.5
printf 'q\n' >&3
exec 3>&-
sleep 0.3
...
for expected in 'LONG 42.000000' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing integrated long-name output: %s\n' "$expected" >&2
        exit 1
    fi
done
printf 'guest assembly REPL long-name QEMU test passed\n'
