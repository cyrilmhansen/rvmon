...
for expected in 'ERR' 'READY>'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing ON error output: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'BAD' "$output_file"; then
    cat "$output_file"
    printf 'invalid ON selector executed a target\n' >&2
    exit 1
fi
printf 'guest assembly REPL ON selector error QEMU test passed\n'
