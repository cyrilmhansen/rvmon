...
sleep 15
printf '%s\n' 'LET S$="HELLO"' 'LET LONGTEXT$="RV64"' '10 PRINT LEN(S$)+LEN(LONGTEXT$)' '20 PRINT LEN("R"+"V64")' '30 PRINT LEN(LONGTEXT$+"!")' '40 END' 'RUN' 'q' |
    while IFS= read -r line; do
        printf '%s\n' "$line" >&3
        sleep 0.15
    done
...
if ! grep -aFq -- '4.000000' "$output_file" ||
    ! grep -aFq -- '5.000000' "$output_file" ||
    ! grep -aFq -- 'trap: breakpoint' "$output_file" ||
    grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'LEN string-concatenation source test failed\n' >&2
    exit 1
fi
printf 'guest assembly REPL LEN string-concatenation QEMU test passed\n'
