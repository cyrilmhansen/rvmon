...
if ! grep -aFq -- '82.000000' "$output_file" ||
    ! grep -aFq -- '90.000000' "$output_file" ||
    ! grep -aFq -- 'trap: breakpoint' "$output_file" ||
    grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'ASC string-concatenation source test failed\n' >&2
    exit 1
fi
printf 'guest assembly REPL ASC string-concatenation QEMU test passed\n'
