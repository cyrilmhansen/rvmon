...
for expected in '33.000000' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing nested FOR output: %s\n' "$expected" >&2
        exit 1
    fi
done
printf 'guest assembly REPL nested FOR QEMU test passed\n'
