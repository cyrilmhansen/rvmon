...
sleep 0.2
printf 'LET LONGSTRINGVALUE$="DIRECT"\nPRINT LONGSTRINGVALUE$\n10 LET LONGSTRINGVALUE$="PROGRAM"\n20 PRINT LONGSTRINGVALUE$\n30 END\nRUN\n' >&3
sleep 0.8
printf 'q\n' >&3
exec 3>&-
sleep 0.3
...
for expected in 'DIRECT' 'PROGRAM' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing long string output: %s\n' "$expected" >&2
        exit 1
    fi
done
printf 'guest assembly REPL long-string QEMU test passed\n'
