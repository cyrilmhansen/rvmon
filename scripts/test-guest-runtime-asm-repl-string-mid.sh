...
printf '%s\n' \
  'LET TEXT$="HAMMURABI"' \
  '10 PRINT MID$(TEXT$,1,4)' \
  '20 PRINT MID$(TEXT$,5,4)' \
  '30 PRINT MID$(TEXT$,9,4)' \
  '40 PRINT MID$(TEXT$,99,4)' \
  '50 PRINT MID$(TEXT$,8,99)' \
  '60 END' \
...
for expected in 'HAMM' 'URAB' 'I' 'BI' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing MID$ result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'unexpected MID$ diagnostic\n' >&2
    exit 1
fi
printf 'guest assembly REPL MID$ QEMU test passed\n'
