...
sleep 15
printf '%s\n' '10 PRINT VAL("12"+"x")' '20 PRINT VAL("")' '30 END' 'RUN' |
    while IFS= read -r line; do
        printf '%s\n' "$line" >&3
        sleep 0.15
    done
...
if (( $(grep -aF -- 'ERR' "$output_file" | wc -l) < 2 )) ||
    ...
    cat "$output_file"
    printf 'VAL string-concatenation error test failed\n' >&2
    exit 1
fi
printf 'guest assembly REPL VAL string-concatenation error QEMU test passed\n'
