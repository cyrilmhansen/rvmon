...
if ! grep -aFq -- 'trap: breakpoint' "$output_file"; then
    cat "$output_file"
    printf 'missing integrated assembly false IF breakpoint\n' >&2
    exit 1
fi
if grep -aFq -- '15.000000' "$output_file"; then
    cat "$output_file"
    printf 'false integrated assembly IF unexpectedly executed target\n' >&2
    exit 1
fi

printf 'guest assembly REPL false IF QEMU test passed\n'
