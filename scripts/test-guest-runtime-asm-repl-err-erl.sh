...
for expected in '1.000000' '10.000000' 'ERR'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing ERR/ERL result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'mcause=' "$output_file"; then
    cat "$output_file"
    printf 'unexpected ERR/ERL target fault\n' >&2
    exit 1
fi
printf 'guest assembly REPL ERR/ERL QEMU test passed\n'
