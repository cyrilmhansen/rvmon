...
for expected in 'THEN-OK' 'ELSE-OK' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing IF block output: %s\n' "$expected" >&2
        exit 1
    fi
done
for forbidden in 'BAD-' 'ERR'; do
    if grep -aFq -- "$forbidden" "$output_file"; then
        cat "$output_file"
        printf 'unexpected IF block result: %s\n' "$forbidden" >&2
        exit 1
    fi
done
printf 'guest assembly REPL IF/ELSE/ENDIF QEMU test passed\n'
