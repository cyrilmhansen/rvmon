...
printf '%s\n' \
  'LET TEXT$="HAMMURABI"' \
  '10 PRINT RIGHT$(TEXT$,4)' \
  '20 PRINT RIGHT$(TEXT$,99)' \
  '30 PRINT RIGHT$(TEXT$,0)' \
  '40 END' \
...
for expected in 'RABI' 'HAMMURABI' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing RIGHT$ result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'unexpected RIGHT$ diagnostic\n' >&2
    exit 1
fi
printf 'guest assembly REPL RIGHT$ QEMU test passed\n'
