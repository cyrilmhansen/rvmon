...
if (( error_count < 3 )); then
    cat "$output_file"
    printf 'expected three numeric-function diagnostics, got %s\n' "$error_count" >&2
    exit 1
fi
if ! grep -aFq -- 'trap: breakpoint' "$output_file"; then
    cat "$output_file"
    printf 'missing clean stop after numeric-function errors\n' >&2
    exit 1
fi
printf 'guest assembly REPL ABS/SGN/INT error QEMU test passed\n'
