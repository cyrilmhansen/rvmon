...
for expected in '8.000000' '7.000000' '-3.000000' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing RAND result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'ERR' "$output_file" || grep -aFq -- 'mcause=' "$output_file"; then
    cat "$output_file"
    printf 'unexpected RAND diagnostic or target fault\n' >&2
    exit 1
fi
printf 'guest assembly REPL RAND QEMU test passed\n'
