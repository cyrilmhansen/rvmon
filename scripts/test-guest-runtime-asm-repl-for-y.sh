...
for expected in \
    ...
    '3.000000' \
    'trap: breakpoint' \
    'f3=0x4008000000000000'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing integrated assembly FOR/NEXT Y output: %s\n' "$expected" >&2
        exit 1
    fi
done

printf 'guest assembly REPL FOR/NEXT Y QEMU test passed\n'
