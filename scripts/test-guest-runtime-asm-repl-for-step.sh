...
cd "$repo_root"
for_program="${FOR_PROGRAM:-10 FOR X=1 TO 5 STEP 2}"
for_expected="${FOR_EXPECTED:-1.000000 3.000000 5.000000}"
...
for expected in $for_expected 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing integrated assembly FOR STEP output: %s\n' "$expected" >&2
        exit 1
    fi
done
printf 'guest assembly REPL FOR STEP QEMU test passed\n'
