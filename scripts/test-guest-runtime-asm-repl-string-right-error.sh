...
if (( error_count < 3 )); then
    cat "$output_file"
    printf 'expected three RIGHT$ diagnostics, got %s\n' "$error_count" >&2
    exit 1
fi
if ! grep -aFq -- 'trap: breakpoint' "$output_file"; then
    cat "$output_file"
    printf 'missing clean stop after RIGHT$ errors\n' >&2
    exit 1
fi
printf 'guest assembly REPL RIGHT$ error QEMU test passed\n'
