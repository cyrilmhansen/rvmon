...
for expected in '5.000000' '4.000000' '3.000000' '9.000000' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing LEN result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'unexpected LEN diagnostic\n' >&2
    exit 1
fi
printf 'guest assembly REPL LEN QEMU test passed\n'
