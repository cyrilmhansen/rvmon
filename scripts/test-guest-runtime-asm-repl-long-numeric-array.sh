...
sleep 0.2
printf 'DIM LONGNUM(2)\nLONGNUM(1)=7\n10 LET LONGNUM(1)=LONGNUM(1)+5\n20 PRINT LONGNUM(1),LONGNUM(0)\n30 END\nRUN\n' >&3
sleep 1.0
printf 'q\n' >&3
exec 3>&-
sleep 0.3
...
for expected in '12.000000' '0.000000' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing long numeric-array output: %s\n' "$expected" >&2
        exit 1
    fi
done
printf 'guest assembly REPL long-numeric-array QEMU test passed\n'
