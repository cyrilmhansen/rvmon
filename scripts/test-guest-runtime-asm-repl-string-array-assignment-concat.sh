...
for expected in 'RABI!' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing string-array assignment-concat result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'ERR' "$output_file" || grep -aFq -- 'mcause=' "$output_file"; then
    cat "$output_file"
    printf 'unexpected string-array assignment-concat diagnostic or target fault\n' >&2
    exit 1
fi
printf 'guest assembly REPL string-array assignment-concat QEMU test passed\n'
