...
sleep 0.2
printf 'DIM LONGGRID(2,3)\nI=1\nJ=2\nLONGGRID(I+0,J-0)=7\n10 LET LONGGRID(0+0,1+0)=LONGGRID(I-0,J-0)+5\n20 PRINT LONGGRID(0+0,1+0),LONGGRID(I-0,J-0),LONGGRID(2,3)\n30 END\nRUN\n' >&3
sleep 1.0
printf 'q\n' >&3
exec 3>&-
sleep 0.3
...
for expected in '12.000000' '7.000000' '0.000000' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing long numeric-array 2d output: %s\n' "$expected" >&2
        exit 1
    fi
done
printf 'guest assembly REPL long-numeric-array-2d QEMU test passed\n'
