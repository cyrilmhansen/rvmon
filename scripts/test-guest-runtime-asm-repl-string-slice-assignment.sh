...
printf '%s\n' \
  'LET TEXT$="HAMMURABI"' \
  'LET SUFFIX$=RIGHT$(TEXT$,4)' \
  'LET MIDDLE$=MID$(TEXT$,2,4)' \
  '10 PRINT SUFFIX$' \
  '20 PRINT MIDDLE$' \
  '30 LET SELF$=RIGHT$(TEXT$,4)' \
  '40 LET SELF$=MID$(SELF$,2,2)' \
  '50 PRINT SELF$' \
  '60 END' \
...
for expected in 'RABI' 'AMMU' 'AM' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing string-slice assignment result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'unexpected string-slice assignment diagnostic\n' >&2
    exit 1
fi
printf 'guest assembly REPL RIGHT$/MID$ assignment QEMU test passed\n'
