...
for expected in 'POP-OK' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing POP output: %s\n' "$expected" >&2
        exit 1
    fi
done
for forbidden in 'BAD-RETURN' 'ERR'; do
    if grep -aFq -- "$forbidden" "$output_file"; then
        cat "$output_file"
        printf 'unexpected POP result: %s\n' "$forbidden" >&2
        exit 1
    fi
done
printf 'guest assembly REPL POP control-stack QEMU test passed\n'
