...
for expected in \
    ...
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing numeric-rounding result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'unexpected numeric-rounding diagnostic\n' >&2
    exit 1
fi
printf 'guest assembly REPL ABS/SGN/INT QEMU test passed\n'
