...
sleep 0.2
printf 'DIM LONGNUM(12)\nLONGNUM(0)=7\nLONGNUM(1)=5\nLONGNUM(11)=9\n10 PRINT LONGNUM(0+1)\n20 PRINT LONGNUM(1-1)\n30 PRINT LONGNUM(10+1)\n40 END\nRUN\n' >&3
sleep 1.0
printf 'q\n' >&3
exec 3>&-
sleep 0.3
...
for expected in '5.000000' '7.000000' '9.000000' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing array-index-expression output: %s\n' "$expected" >&2
        exit 1
    fi
done
printf 'guest assembly REPL array-index-expression QEMU test passed\n'
