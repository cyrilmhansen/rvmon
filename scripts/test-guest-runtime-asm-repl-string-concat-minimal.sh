...
if ! grep -aFq -- 'RABI<' "$output_file"; then
    cat "$output_file"
    printf 'missing slice/literal concatenation result\n' >&2
    exit 1
fi
if grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'unexpected long-variable concatenation diagnostic\n' >&2
    exit 1
fi
printf 'guest assembly REPL minimal long-variable concatenation QEMU test passed\n'
