...
exec 3>"$input_fifo"
sleep 0.1
sed '/^regs$/,$d' examples/minibasic-runtime-command-run-variable-mul.rv >&3
printf 'RUN\n' >&3
sleep 0.2
printf 'regs\nmemory 0x82000400 8\nq\n' >&3
exec 3>&-
...
for expected in \
    ...
    if ! grep -aFq "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing target variable multiplication output: %s\n' "$expected" >&2
        exit 1
    fi
done

printf 'guest RUN variable multiplication QEMU test passed\n'
