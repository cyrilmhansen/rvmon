...
sleep 0.3
printf '%s\n' \
  'LET PREFIX$=LEFT$("HAMMURABI",4)' \
  'LET SUFFIX$=RIGHT$("HAMMURABI",4)' \
  'DIM A$(1)' \
  'LET A$(0)=MID$("HAMMURABI",2,4)' \
  '10 PRINT PREFIX$' \
  '20 PRINT SUFFIX$' \
  '30 PRINT A$(0)' \
  '40 PRINT LEFT$("MESOPOTAMIA",4)' \
  '50 END' \
...
for expected in 'HAMM' 'RABI' 'AMMU' 'MESO' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing literal-source slice result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'unexpected literal-source slice diagnostic\n' >&2
    exit 1
fi
printf 'guest assembly REPL string slice literal-source QEMU test passed\n'
