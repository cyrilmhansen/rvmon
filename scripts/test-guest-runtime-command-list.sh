...
exec 3>"$input_fifo"
sleep 0.1
sed '/^q$/,$d' examples/minibasic-runtime-command-list.rv >&3
printf 'LIST\n' >&3
sleep 0.2
printf 'q\n' >&3
exec 3>&-
...
if ! grep -aFq $'10 PRINT A\n20 PRINT B\n' "$output_file" || \
    ! grep -aFq 'target exit status=0' "$output_file"; then
    cat "$output_file"
    printf 'missing target LIST command output\n' >&2
    exit 1
fi

printf 'guest LIST command QEMU test passed\n'
