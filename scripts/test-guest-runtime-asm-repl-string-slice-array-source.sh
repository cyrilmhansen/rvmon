...
printf '%s\n' \
  'DIM A$(2)' \
  'A$(0)="HAMMURABI"' \
  'LET SUFFIX$=RIGHT$(A$(0),4)' \
  'LET MIDDLE$=MID$(A$(0),2,4)' \
  'DIM LONGARRAY$(2)' \
  'LONGARRAY$(1)="MESOPOTAMIA"' \
  'LET PREFIX$=LEFT$(LONGARRAY$(1),4)' \
  '10 PRINT SUFFIX$' \
  '20 PRINT MIDDLE$' \
  '30 PRINT PREFIX$' \
  '40 END' \
...
for expected in 'RABI' 'AMMU' 'MESO' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing array-source slice result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'unexpected array-source slice diagnostic\n' >&2
    exit 1
fi
printf 'guest assembly REPL string slice array-source QEMU test passed\n'
