...
sleep 0.3
printf '%s\n' \
  'LET OUT$=LEFT$("HAMMURABI",-1)' \
  'LET OUT$=RIGHT$("HAMMURABI",121)' \
  'LET OUT$=MID$("HAMMURABI",2)' \
  'LET OUT$=LEFT$("UNTERMINATED,4)' \
  '10 END' \
...
if (( error_count < 4 )); then
    cat "$output_file"
    printf 'expected four literal-source slice diagnostics, got %s\n' "$error_count" >&2
    exit 1
fi
if ! grep -aFq -- 'trap: breakpoint' "$output_file"; then
    cat "$output_file"
    printf 'missing clean stop after literal-source slice errors\n' >&2
    exit 1
fi
printf 'guest assembly REPL string slice literal-source error QEMU test passed\n'
