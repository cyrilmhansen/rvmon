...
if (( error_count < 5 )); then
    cat "$output_file"
    printf 'expected five string-slice assignment diagnostics, got %s\n' "$error_count" >&2
    exit 1
fi
if ! grep -aFq -- 'trap: breakpoint' "$output_file"; then
    cat "$output_file"
    printf 'missing clean stop after string-slice assignment errors\n' >&2
    exit 1
fi
printf 'guest assembly REPL RIGHT$/MID$ assignment error QEMU test passed\n'
