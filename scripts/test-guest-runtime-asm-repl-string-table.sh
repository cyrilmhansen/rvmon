...
for expected in 'PROGRAM ALPHA' 'PROGRAM BETA' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing string table output: %s\n' "$expected" >&2
        exit 1
    fi
done
printf 'guest assembly REPL string table QEMU test passed\n'
