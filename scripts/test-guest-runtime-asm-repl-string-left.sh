...
printf '%s\n' \
  'LET TEXT$="HAMMURABI"' \
  '10 PRINT LEFT$(TEXT$,4)' \
  '20 PRINT LEFT$(TEXT$,99)' \
  '30 PRINT LEFT$(TEXT$,0)' \
  '40 END' \
...
for expected in 'HAMM' 'HAMMURABI' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing LEFT$ result: %s\n' "$expected" >&2
        exit 1
    fi
done
if grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'unexpected LEFT$ diagnostic\n' >&2
    exit 1
fi
printf 'guest assembly REPL LEFT$ QEMU test passed\n'
