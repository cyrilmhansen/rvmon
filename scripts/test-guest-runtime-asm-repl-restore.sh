...
for expected in '7.000000' 'trap: breakpoint'; do
    if ! grep -aFq -- "$expected" "$output_file"; then
        cat "$output_file"
        printf 'missing RESTORE output: %s\n' "$expected" >&2
        exit 1
    fi
done
if [[ "$(grep -aFo '7.000000' "$output_file" | wc -l)" -lt 2 ]]; then
    cat "$output_file"
    printf 'RESTORE did not replay DATA\n' >&2
    exit 1
fi
printf 'guest assembly REPL RESTORE QEMU test passed\n'
