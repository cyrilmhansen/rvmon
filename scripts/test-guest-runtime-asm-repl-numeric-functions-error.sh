...
if ! grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'missing numeric-function error\n' >&2
    exit 1
fi
if ! grep -aFq -- 'trap: breakpoint' "$output_file"; then
    cat "$output_file"
    printf 'missing clean stop after numeric-function error\n' >&2
    exit 1
fi
printf 'guest assembly REPL numeric functions error QEMU test passed\n'
