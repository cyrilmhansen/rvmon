...
if ! grep -aFq -- 'ERR' "$output_file"; then
    cat "$output_file"
    printf 'missing orphan ELSE diagnostic\n' >&2
    exit 1
fi
if ! grep -aFq -- 'trap: breakpoint' "$output_file"; then
    cat "$output_file"
    printf 'missing clean return after orphan ELSE diagnostic\n' >&2
    exit 1
fi
printf 'guest assembly REPL IF block error QEMU test passed\n'
