...
pub struct Memory { bytes: Vec<u8> }

impl Memory {
    pub fn new(size: usize) -> Self { Self { bytes: vec![0; size] } }
    fn range(&self, address: u64, width: usize) -> Result<std::ops::Range<usize>> {
        let start = usize::try_from(address).map_err(|_| Diagnostic::error("MEM-ADDRESS-001", "address does not fit host index"))?;
        let end = start.checked_add(width).ok_or_else(|| Diagnostic::error("MEM-ADDRESS-002", "address range overflow"))?;
        if end > self.bytes.len() { return Err(Diagnostic::error("MEM-ACCESS-001", "target memory access is unmapped")); }
        Ok(start..end)
    }
