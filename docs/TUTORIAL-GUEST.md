...
| 5 | `poll_char` | aucune | `a0=0` si vide, sinon octet reçu et consommé de la file RX |
| 6 | `poll_break` | aucune | `a0=0` si aucun Ctrl-C, sinon `3`; les octets ordinaires restent dans la file |
...
de MiniBASIC. `poll_break` est non bloquant et permet au dispatcher de vérifier
périodiquement cette interruption sans voler un caractère destiné à
`INKEY$()` ; `poll_char` est la primitive consommante utilisée par
`INKEY$()`. Un service inconnu produit
...
commande `assemble-program` accepte 12288 lignes, avec 2048 labels. Cette
