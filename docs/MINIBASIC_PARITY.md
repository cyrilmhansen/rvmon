...
État de l’audit au 3 août 2026 : **99 scripts QEMU assembleur MiniBASIC
recensés**. Ce nombre
est contrôlable par :
...
| Fonctions mathématiques historiques non retenues dans la grammaire | Différées, pas implicitement supportées | DIFFÉRÉ | pas encore de `ATN` dans V1 actuelle |
...
4. auditer les familles TBXL non encore retenues (fonction trigonométrique
   inverse, formatages et commandes d’édition) et
...
- **Décisions de parité figées :** mode direct, lignes, contrôle de flot,
  ...
  rejeté ;
- **Preuve automatisée :** 99 tests QEMU assembleur recensés au moment de cet
