...
| Horloge | `TIME` target-side déterministe ; `TIME$` et réglage de l’heure restent différés | PARTIEL | `test-guest-runtime-asm-repl-time.sh` sous QEMU : deux lignes renvoient `1.000000` puis `2.000000` |
...
| 2026-08-04 | Ajout de `TIME` target-side déterministe | `test-guest-runtime-asm-repl-time.sh` sous QEMU : `PRINT TIME` sur deux lignes produit `1.000000` puis `2.000000`, sans fault | le compteur par statement respecte le déterminisme RVMonitor ; la cadence historique 60 Hz, `TIME$` et l’écriture de l’heure restent explicitement différés |
