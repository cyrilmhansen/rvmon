...
État de l’audit : **76 scripts QEMU assembleur MiniBASIC recensés**. Le nombre
est contrôlable par :
...
| Fonctions de chaînes | `LEN` target-side ; `PRINT LEFT$(variable,n)` avec buffer target-side | PARTIEL | `string-len*.sh`, `string-left*.sh`; affectations, tableaux, littéraux, `RIGHT$` et `MID$` différés |
...
3. étendre les fonctions chaîne target-side avec `RIGHT$` puis `MID$`, puis
   décider l’activation dans les affectations, les tableaux et les littéraux ;
   `LEN` et `PRINT LEFT$` sont désormais implémentés avec les limites
   documentées ;
...
- **Décisions de parité figées :** mode direct, lignes, contrôle de flot,
  ...
  rejeté ;
- **Preuve automatisée :** 76 tests QEMU assembleur recensés au moment de cet
  audit ;
