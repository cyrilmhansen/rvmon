...
| Fonctions de chaînes | `LEN`, `PRINT LEFT$`, `PRINT RIGHT$` et `PRINT MID$` avec buffer target-side ; les trois découpes d’affichage acceptent l’expression chaîne commune, tandis que les affectations restent bornées aux sources simples | PARTIEL | `string-len*.sh`, `test-guest-runtime-asm-repl-string-len-concat.sh`, `test-guest-runtime-asm-repl-string-slice-expression.sh`, `string-left*.sh`, `string-right*.sh`, `string-mid*.sh`, `string-slice-array-source.sh`, `string-slice-literal-source*.sh` |
...
| `LEN`, `LEFT$`, `RIGHT$`, `MID$` | Conservées avec sources/destinations RV étendues et composition par expressions chaîne | PARTIEL | `LEN` et les découpes d’affichage consomment le résolveur commun ; les affectations de découpe et certaines imbrications de concaténation restent bornées |
...
| 2026-08-04 | Les découpes d’affichage consomment une expression chaîne composée | `test-guest-runtime-asm-repl-string-slice-expression.sh` sous QEMU : `LEFT$(TEXT$+"X",5)`, `RIGHT$("0"+TEXT$,4)` et `MID$(TEXT$+"X",2,4)` | le résolveur target-side commun s’arrête à la virgule de niveau zéro ; les affectations de découpe restent explicitement PARTIELLES après une régression détectée sur `RIGHT$` concaténé |
