...
| Horloge | `TIME` et `TIME$` target-side déterministes ; réglage par `LET` et forme directe | PARTIEL | `test-guest-runtime-asm-repl-time.sh` sous QEMU : lecture, affectation `LET`, formatage et concaténation |
