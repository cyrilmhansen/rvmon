...
| Conversion nombre → chaîne | `STR$(expression)` target-side, format fixe à six décimales réutilisable en affectation/concaténation | PARTIEL | `test-guest-runtime-asm-repl-string-concat.sh`, `test-guest-runtime-asm-repl-string-concat-error.sh`; `PRINT STR$` direct et formats historiques différés |
...
| 2026-08-03 | Formatage `STR$(expression)` dans un buffer chaîne target-side | mêmes scénarios QEMU, valeurs positives/négatives et argument absent | le formateur partage la politique fixe à six décimales de `PRINT` sans délégation hôte |
