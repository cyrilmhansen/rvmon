...
État de l’audit au 4 août 2026 : **124 scripts QEMU assembleur MiniBASIC
...
- **Décisions de parité figées :** mode direct, lignes, contrôle de flot,
  ...
  rejeté ;
- **Preuve automatisée :** 124 scripts QEMU assembleur recensés au moment de
...
| 2026-08-04 | Audit de cohérence du registre | comptage reproductible : `find scripts -maxdepth 1 -type f -name 'test-guest-runtime-asm-repl*.sh' \| wc -l` → `124` | l’inventaire courant est distingué des comptes historiques ; les preuves de fonctions chaîne imbriquées, `HEX$` et `DEC` sont incluses |
