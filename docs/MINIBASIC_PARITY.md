...
| 2026-08-03 | Migration directe du handler `INPUT` via l'ID 9 | `test-guest-runtime-asm-repl-input.sh`, `long-input.sh` et `long-string-input.sh` sous QEMU | `INPUT` restaure le contexte puis appelle `input_statement`, qui conserve la lecture console et la conversion target-side ; les bornes de longueur restent celles documentées |
