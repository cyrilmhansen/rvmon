...
État de l’audit au 4 août 2026 : **127 scripts QEMU assembleur MiniBASIC
...
| Diagnostics `ERR()` / `ERL()` | code `1` et dernière ligne target-side, requête après une faute récupérable | PARTIEL | `test-guest-runtime-asm-repl-err-erl.sh` sous QEMU ; les codes détaillés restent à généraliser |
...
| Lecture clavier non bloquante | `INKEY$()` retourne une chaîne vide ou consomme un octet depuis `poll-char`, entièrement dans le guest | VERT | `test-guest-runtime-asm-repl-string-inkey.sh` sous QEMU ; un programme cible attend effectivement un octet `K` et atteint `GOT` |
...
| Recherche chaîne | `INSTR` et `UINSTR(haystack,needle[,start])` target-side, résultat 1-based, `0` absent et `start` pour aiguille vide ; `UINSTR` compare les lettres ASCII sans casse | PARTIEL | `test-guest-runtime-asm-repl-string-instr-expression.sh`, `test-guest-runtime-asm-repl-uinstr.sh`, `test-guest-runtime-asm-repl-string-concat.sh`, `test-guest-runtime-asm-repl-string-concat-error.sh`; `start` est un littéral décimal positif en V1 et les formats historiques restent hors contrat |
...
| Aléatoire | `RND`, `RND()`, alias historique `RND(0)` et `RAND(n)`, LCG target-side reproductible, graine à 1 | VERT | `rnd.sh`, `rnd-error.sh`, `rand.sh`, `rand-error.sh` |
| Horloge | `TIME`, `TIME$`, réglage par `LET`/forme directe et `PAUSE` target-side déterministes | PARTIEL | `test-guest-runtime-asm-repl-time.sh` sous QEMU : lecture, affectation, formatage, concaténation et avance virtuelle |
...
| `ABS`, `SGN`, `INT`, `TRUNC`, `FRAC`, `MOD`, `SQR`, `SIN`, `COS`, `TAN`, `LOG`, `EXP`, `RND`, `RAND` | Sous-ensemble numérique explicite | VERT/PARTIEL | `SQR` utilise `fsqrt.d`, les fonctions transcendantes utilisent réduction et polynômes target-side ; `LOG/EXP` sont bornées et leur approximation fixe peut perdre une unité au dernier chiffre affiché |
...
| 2026-08-04 | Matrice de références RENUM | `test-guest-runtime-asm-repl-renum-reference-matrix.sh` sous QEMU ; `GOSUB`, `ON ... GOSUB`, `IF ... THEN`, plusieurs cibles et nombres dans une chaîne littérale | les formes principales de réécriture de contrôle de flot sont maintenant couvertes ensemble ; les limites de capacité et de syntaxe restent explicites |
...
| 2026-08-04 | Audit de cohérence du registre | comptage reproductible : `find scripts -maxdepth 1 -type f -name 'test-guest-runtime-asm-repl*.sh' \| wc -l` → `127` | l’inventaire courant est distingué des comptes historiques ; les preuves de fonctions chaîne imbriquées, `HEX$`, `DEC`, `INKEY$`, `ERR/ERL` et `TIME` sont incluses |
...
| 2026-08-04 | Ajout de `INKEY$()` target-side | `test-guest-runtime-asm-repl-string-inkey.sh` sous QEMU : boucle `IF INKEY$()<>"" THEN 40`, octet UART `K`, sortie `GOT` et absence de fault | une fonction TBXL de lecture clavier est modernisée vers `poll-char`, entièrement target-side ; `poll-break` sépare le sondage Ctrl-C pour ne pas voler l’octet |
| 2026-08-04 | Ajout de `ERR()` / `ERL()` target-side et réarmement après faute `RUN` | `test-guest-runtime-asm-repl-err-erl.sh` sous QEMU : `GOTO 999`, puis `?ERR(),ERL()` produit `1.000000` et `10.000000`, sans fault | la compatibilité de diagnostic est bornée au code `1` en V1 ; le numéro de ligne est conservé dans la RAM cible et la requête est exécutée après retour à `READY>` |
| 2026-08-04 | Ajout de `TIME` et `TIME$` target-side déterministes | `test-guest-runtime-asm-repl-time.sh` sous QEMU : `TIME`, `TIME$`, concaténation, affectation et arrêt propre | le compteur par statement respecte le déterminisme RVMonitor ; `TIME$` applique localement 60 ticks/seconde au format `HHMMSS` et conserve une base réglable dans la RAM cible |
| 2026-08-04 | Compatibilité `RND(0)` | `test-guest-runtime-asm-repl-rnd.sh` sous QEMU : `RND`, `RND()`, `RND(0)` et nouvelle séquence déterministe | l’ancien argument factice TBXL est accepté dans la cible ; `RND(1)` reste explicitement rejeté |
| 2026-08-04 | Ajout de `RAND(n)` target-side | `test-guest-runtime-asm-repl-rand.sh` et `rand-error.sh` sous QEMU : bornes positives/négatives, fraction, zéro et séquence LCG | le générateur entier historique TBXL est exécuté dans la cible et partage l’état de `RND` ; les domaines invalides retournent une erreur sans fault |
| 2026-08-04 | Ajout de `UINSTR` target-side | `test-guest-runtime-asm-repl-uinstr.sh` sous QEMU : casse mixte, absence et aiguille vide | la recherche insensible à la casse reste dans les buffers de la cible et conserve le contrat 1-based de `INSTR` |
| 2026-08-04 | Position de départ optionnelle pour `INSTR` et `UINSTR` | `test-guest-runtime-asm-repl-uinstr.sh` sous QEMU : recherches avec `start=3`, `start=5`, position absente et aiguille vide | le littéral décimal est parsé dans le guest, validé comme entier positif et ne modifie pas le contrat par défaut `start=1` ; les expressions générales restent différées |
