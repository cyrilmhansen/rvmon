...
`GOTO`, `IF`, `ELSE`, `ENDIF`, `INPUT`, `PRINT`, `END` et `REM` sont maintenant les premiers handlers de statements appelés
...
`goto_dispatch`, `if_statement`, `else_statement`, `endif_statement`, `input_statement` ou `end_statement`; `REM` restaure le même contexte avant de
terminer la ligne.
