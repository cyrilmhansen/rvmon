...
Le dossier `docs/release/` est contrôlé par le même audit : manifest du commit
et de l’arbre d’entrée, hash du toolchain et de R2, SBOM TSV issu de Cargo et
notice de licences. `tools/generate-release-dossier.sh --check` échoue si les
artefacts commis ne correspondent plus au checkout.

La campagne guest/QEMU unifiée se lance avec :
...
| luna-monitor | 52 | parseur partagé de commandes, validation d’arité `CMD-002`, plages inversées `CMD-004`, AST d’expressions signées, modèle curseur/sélection, rendu hex/ASCII/CP437 partagé, `find`/`fill`/`copy` transactionnels, modèle et édition exacte des registres `x`/`f`, aliases ABI, protection x0, édition contrôlée de `fcsr`/`frm`/`fflags`, deltas exacts `x`/`f`/`fcsr` après step et dans l’historique, surlignage `*` depuis la baseline dans host/backend, dashboard `all` et sélection `location`/`regs`/`blinkenlights`/`memory` host/backend, source numérotée, `source replace` non destructif, dernier diagnostic, caret et remède d’erreur, adresses `pc`/registres/marques, diagnostics `CMD-*`, assemble → step → regs, affichage flottant, run borné, vues mémoire, désassemblage mixte code/données/C, édition/undo, console backend-générique, marques QuickJump, breakpoints, watchpoints, symboles, pile, historique et persistance. |
...
| luna-guest-monitor | 0 | Image bare-metal, boot QEMU, FIFO RX/TX NS16550, tampon logiciel de réception de 4096 octets avec polling borné et IRQ PLIC M-mode pendant U-mode, patch binaire brut/RLE et bornes de décompression, exposition bornée du metadata `RVMETA01`, PMP, transition M→U, lecture/édition mémoire transactionnelle, `undo`, directives exactes `.word`/`.float`/`.binary128`, assemblage invité entier/flottant, diagnostics `GUEST-ASM-*` avec ligne source, source persistante/correction non destructive et `assemble-source`, buffer source assembleur de 256 lignes/128 caractères avec 64 labels et scratch statique hors pile, snapshots/projets volatils des régions guest, `snapshot info/manifest/dump/patch` par blocs UART, CRC-32 modifié après patch et erreurs de bornes, `run` borné et budgets invalides, watchpoints logiciels `ld`/`sd` avant écriture, `set`/`setf`, NaN-boxing, `fmv.w.x`/`fmv.x.w` et traps; vérifié par E2E UART QEMU hors comptage Cargo. |
| luna-snapshot-format | 9 | Encode/decode RVSNAP01 et RVPROJ01, metadata `RVMETA01` bit-exact, CRC-32, limites de régions, corruption/troncature, collecte générique des blocs guest, collecte metadata hexadécimale, application binaire bornée et refus des manifestes/chunks incohérents; export TCP QEMU réel et patchbin guest vérifiés par `scripts/test-guest-snapshot-export.sh` et `scripts/test-guest-snapshot-binary.sh`. |
| luna-app | 3 | Rappel borné des commandes host/QEMU avec `!!` et `!N`, références invalides et codes `APP-SHELL-*`; éditeur TTY crossterm, raccourcis `F5=run`, `F10=step-over`, `F11=step-out` et `Ctrl+1/2/3`; chemin script conservé. |
...
2. `stepidp 2` rend chaque état complet, `fcsr`, la fenêtre de pile et le bloc mémoire d’un store ; un test dédié vérifie aussi les annotations appel/branche/retour ;
3. assemble fadd.s, step, puis le motif flottant et fcsr ;
4. run 3 sur une boucle et respect de la borne.
...
Le benchmark smoke `tools/bench-smoke.sh` mesure 10 échantillons de 100 cycles
assemble/step/snapshot en build release sur une machine donnée et affiche p50,
