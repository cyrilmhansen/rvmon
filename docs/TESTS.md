...
| luna-snapshot-format | 5 | Encode/decode RVSNAP01, CRC-32, limites de régions, corruption/troncature, collecte générique des blocs guest, application binaire bornée et refus des manifestes/chunks incohérents; export TCP QEMU réel et patchbin guest vérifiés par `scripts/test-guest-snapshot-export.sh` et `scripts/test-guest-snapshot-binary.sh`. |
