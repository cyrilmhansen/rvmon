...
| luna-guest-monitor | 0 | Image bare-metal, boot QEMU, FIFO RX/TX NS16550 et tampon logiciel de réception de 64 octets avec polling borné, patch binaire brut/RLE et bornes de décompression, exposition bornée du metadata `RVMETA01`, PMP, transition M→U, lecture/édition mémoire transactionnelle, `undo`, directives exactes `.word`/`.float`/`.binary128`, assemblage invité entier/flottant, diagnostics `GUEST-ASM-*` avec ligne source, source persistante/correction non destructive et `assemble-source`, snapshots/projets volatils des régions guest, `snapshot info/manifest/dump/patch` par blocs UART, CRC-32 modifié après patch et erreurs de bornes, `run` borné et budgets invalides, watchpoints logiciels `ld`/`sd` avant écriture, `set`/`setf`, NaN-boxing, `fmv.w.x`/`fmv.x.w` et traps; vérifié par E2E UART QEMU hors comptage Cargo. |
