...
object="$temp_dir/fp-integer-probe.o"
binary="$temp_dir/fp-integer-probe"
...
riscv64-linux-gnu-as \
    ...
    -mabi=lp64d \
    -o "$object" \
    tests/oracles/fp_integer_qemu_probe.S
...
machine_hex="$(cargo run -q -p luna-machine --example fp_integer_probe 2>/dev/null)"
if [[ "$qemu_hex" != "$machine_hex" ]]; then
    printf 'floating integer-conversion oracle mismatch\nQEMU:    %s\nMachine: %s\n' \
