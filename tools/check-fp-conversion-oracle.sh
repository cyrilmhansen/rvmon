...
object="$temp_dir/fp-conversion-probe.o"
binary="$temp_dir/fp-conversion-probe"
...
riscv64-linux-gnu-as \
    ...
    -mabi=lp64d \
    -o "$object" \
    tests/oracles/fp_conversion_qemu_probe.S
...
machine_hex="$(cargo run -q -p luna-machine --example fp_conversion_probe 2>/dev/null)"
if [[ "$qemu_hex" != "$machine_hex" ]]; then
    printf 'floating conversion oracle mismatch\nQEMU:    %s\nMachine: %s\n' \
        "$qemu_hex" "$machine_hex" >&2
    exit 1
fi

printf 'QEMU %s independently matches 3 F/D conversion result-and-flag cases; PASS\n' \
    "$actual_qemu"
