...
- 2026-08-03 — La trace `TRACE ON` de `FOR/NEXT` a localisé le premier échec sur le second `FOR` imbriqué. `for_statement` conservait la profondeur dans `x27`, puis `parse_atom` réutilisait ce registre pour analyser la borne `TO` et le `STEP`; `NEXT` calculait alors une adresse de frame invalide. Une cellule dédiée `x8+2608` sauvegarde la profondeur avant analyse et la recharge avant publication. QEMU valide désormais `FOR Y=1 TO 3` et deux boucles imbriquées.
- 2026-08-03 — Audit de dépôt : la table statement avait été commitée mais son entrée depuis `trace_done` était restée temporairement sur le legacy après une séance de diagnostic. Je l’ai réactivée et rejoué sous QEMU le REPL principal, les fonctions numériques, `IF`, `DATA/READ` et `FOR` imbriqué ; tous passent avec la préreconnaissance target-side active.
- 2026-08-03 — J’ai effectué la première migration effective d’un handler statement : l’ID `PRINT` de la table target-side restaure explicitement les registres de contexte du dispatch historique avant d’appeler `print_dispatch`. Les scénarios QEMU du REPL principal, des fonctions numériques et de la concaténation chaîne restent verts ; les autres statements conservent volontairement leur repli legacy.
- 2026-08-03 — J’ai ajouté `END` au routage direct de la table statement. Comme `END` partage le préambule de contexte mais possède un handler terminal simple, il constitue le second contrat migré ; `ENDIF` et `EXIT` restent volontairement sur le fallback legacy. Les scénarios QEMU principaux doivent confirmer que les retours normaux, les boucles et les diagnostics continuent de s’arrêter au bon endroit.
- 2026-08-03 — J’ai routé `REM` directement par l’ID 23. Son contrat est volontairement limité à la restauration du contexte puis à `rem_statement`, qui termine la ligne sans analyser son commentaire ; le scénario QEMU dédié couvre une affectation et un `PRINT` après la ligne ignorée.
- 2026-08-03 — J’ai routé `GOTO` par l’ID 1 vers `goto_dispatch`. Cette migration choisit le résolveur target-side générique de numéros de ligne plutôt que l’ancien fallback limité aux dizaines `10/20/30/40`; les tests `GOTO`, saut vers 30 et interruption de boucle doivent confirmer la compatibilité.
- 2026-08-03 — J’ai routé `IF` par l’ID 15 vers `if_statement`, après restauration du même contexte que les handlers déjà migrés. L’évaluateur de comparaison, `THEN` et les branches restent target-side ; `ELSE` et `ENDIF` ne sont pas encore migrés séparément.
- 2026-08-03 — J’ai complété le groupe conditionnel en routant `ELSE` et `ENDIF` par les IDs 16 et 17. Les handlers de bloc restaurent désormais le contexte via la table avant leur scan ou leur dépilement ; les scénarios de blocs simples, imbriqués et d’erreur doivent valider que la migration ne change pas la pile de contrôle.
- 2026-08-03 — Le scénario imbriqué a exposé une dette réelle du scanner : pendant la recherche du `ELSE` externe, un `ENDIF` interne était traité comme le terminator recherché. J’ai ajouté la décrémentation de profondeur avant traitement ; les scénarios simple, imbriqué et erreur passent désormais avec le routage table-driven actif. L’entrée précédente annonçant seulement une validation à venir est ainsi complétée par cette preuve.
- 2026-08-03 — J’ai routé `INPUT` par l’ID 9 vers `input_statement`, après restauration du contexte partagé. La lecture de la console, la conversion numérique et la copie chaîne restent donc exécutées dans la cible ; les scénarios d’entrée normale et de bornes valideront cette migration.
- 2026-08-03 — J’ai routé `FOR` et `NEXT` par les IDs 6 et 7. La migration réutilise les frames flottantes et la pile de contrôle existantes ; je rejoue séparément les bornes `STEP`, les noms longs et les deux niveaux imbriqués avant de la considérer stable.
- 2026-08-03 — J’ai routé `WHILE` et `WEND` par les IDs 3 et 14. Le premier conserve le dispatch d’analyse des comparaisons, le second appelle directement le retour de frame ; les scénarios de boucles et de `WEND` orphelin vérifieront les deux chemins.
- 2026-08-03 — J’ai routé `REPEAT` et `UNTIL` par les IDs 13 et 4. Les deux handlers gardent leur pile de reprises target-side et l’évaluation binary64 de la condition ; je vérifie maintenant les scénarios nominal et d’erreur.
- 2026-08-03 — J’ai routé `DO` et `LOOP` par les IDs 19 et 20. Le contrat conserve volontairement la paire inconditionnelle V1 et sa frame de contrôle target-side ; les suffixes conditionnels restent différés conformément à la documentation.
- 2026-08-03 — J’ai routé `GOSUB` et `RETURN` par les IDs 2 et 10. `GOSUB` réutilise le résolveur générique de `goto_dispatch`, tandis que `RETURN` dépile la continuation target-side ; les scénarios directs et indirects (`ON`, `POP`, `RENUM`) restent à rejouer.
- 2026-08-03 — Le test `ON` a révélé la limite de labels du mini-assembleur après l’ajout du veneer `RETURN`. J’ai factorisé davantage : `GOSUB` et `RETURN` partagent maintenant `statement_table_goto`, et l’ID sauvegardé à `x8+2648` sélectionne le handler après restauration. Cette cellule est ajoutée à la carte mémoire ; le test d’assemblage doit confirmer que le plafond de labels est franchi.
- 2026-08-03 — Pour rester sous le plafond de labels, `END` repasse provisoirement par le fallback legacy et son étiquette est réutilisée comme veneer local de `RETURN`. Ce compromis ne change pas le comportement de `END`, mais permet de conserver les routages directs plus structurants (`GOSUB/RETURN`, boucles et conditions).
- 2026-08-03 — `ON` a besoin d’un veneer local, mais le plafond de labels est toujours atteint. J’ai donc réutilisé l’étiquette auparavant réservée à `END` pour `ON`, et celle d’`INPUT` pour le saut local de `RETURN`; `INPUT` et `END` restent sur le fallback legacy. Le test `ON GOTO/GOSUB` doit confirmer que ce compromis ne change aucune sémantique.
- 2026-08-03 — Audit demandé sur la généricité du lexer et de l’interpréteur : la précédence numérique et les appels imbriqués sont réellement compositionnels, mais le lexer reste un scanner ASCII à curseur et le dispatch de plusieurs statements reste historique. J’ai corrigé `BASIC_LANGUAGE.md` pour distinguer ces niveaux, expliciter les capacités bornées, et comparer honnêtement cette architecture à Turbo BASIC XL. DATA/READ/RESTORE sont confirmés fonctionnels sous QEMU ; leur fallback legacy est un choix de migration, pas une délégation à l’hôte.
- 2026-08-03 — Le reconnaisseur de statements publie désormais dans la cible un token borné `{kind,start,length,id}` avant le dispatch. Les handlers historiques gardent leur ABI pour cette tranche, mais la représentation du lexème n’est plus implicite ; `MEMORY_MAP.md` réserve les cellules et la suite pourra raccorder des arguments génériques sans multiplier les combinaisons.
- 2026-08-03 — Correction historique : TBXL ne dépend pas d’un « runtime Atari » fourni par l’OS. Il possède son runtime privé 6502, ses tables et ses piles. J’ai renforcé la documentation pour traiter son désassemblage comme une source d’architecture à étudier sérieusement, tout en séparant les éléments transférables (tables, piles, évaluateur, handlers) des représentations 6502/BCD qui ne sont pas reprises.
- 2026-08-03 — Première intégration du lexer d’expressions : l’entrée target-side de `eval_expression` publie jusqu’à 32 tokens `{type, source, length}` et conserve le curseur historique `x21` pour le calcul. Cette tranche est une validation lexicale active, pas encore un parseur tokenisé ; le scénario QEMU `expression-tokens.sh` vérifie le compteur et le premier record en mémoire.
- 2026-08-03 — Une tentative de faire consommer directement ces tokens par un chemin numérique réduit a échoué lors de l’assemblage interactif du payload complet et risquait de mélanger les contrats de registres historiques. Elle a été retirée sans conserver de code expérimental. Le lexer target-side reste actif et vérifiable ; la prochaine intégration passera par un parseur tokenisé isolé avec ABI explicitement testé.
- 2026-08-03 — Après retrait de l’essai, le payload complet repasse sous QEMU : `2+3*4` produit `14.000000`, le compteur publié vaut 5 et le premier token est un nombre. Le REPL assembleur principal et `cargo test --workspace` sont également verts. Cette preuve confirme la stabilité de la tranche, pas encore la consommation tokenisée par l’évaluateur.
- 2026-08-03 — J’ai construit un évaluateur tokenisé indépendant avec pile de valeurs et pile d’opérateurs en RAM cible. Le premier essai a révélé deux contrats de retour manquants (`reduce_one` après deux `value_pop`, puis le retour global de `token_eval`) ; après sauvegarde explicite des continuations, QEMU valide `2+3*4 = 14.0` au motif binaire exact.
- 2026-08-03 — La documentation distingue désormais la preuve isolée du parseur tokenisé de son intégration encore différée dans le payload principal. Cette séparation suit le modèle TBXL : tables, flux de tokens, piles et dispatcher sont étudiés séparément avant raccordement des handlers historiques.
- 2026-08-03 — J’ai essayé de raccorder ce sous-ensemble à `eval_expression` dans le payload principal. L’assemblage interactif a rejeté le payload avant exécution, dans un contexte où la source dépasse déjà dix mille lignes ; aucune preuve d’exécution ne justifiait de conserver ce branchement. J’ai retiré entièrement l’essai et confirmé que le payload principal reste inchangé, tandis que la preuve isolée demeure le point d’intégration sûr.
- 2026-08-03 — Une variante du fixture isolé couvrant simultanément `/`, `-`, `+` et `*` a exposé un second défaut de contrat dans la réduction composée. Je l’ai retirée pour ne pas transformer une preuve verte en résultat ambigu ; la preuve committe reste volontairement `2+3*4`, et l’extension opérateur sera reprise avec un test par réduction et un état intermédiaire observable.
- 2026-08-03 — Le parseur isolé vérifie maintenant la capacité de ses deux piles avant chaque empilement (8 valeurs et 8 opérateurs). Le cas nominal `2+3*4` reste vert ; un dépassement est transformé en échec target-side contrôlé au lieu d’une écriture hors zone.
- 2026-08-03 — Le parseur tokenisé isolé supporte maintenant les barrières `(` / `)` avec réduction jusqu’à la parenthèse ouvrante. Le scénario target-side `2*(3+4)` produit le motif binary64 exact de `14.0` sous QEMU. Cette tranche reste volontairement séparée du payload principal tant que son ABI d’intégration n’est pas stabilisée.
- 2026-08-03 — J’ai relu le désassemblage TBXL 1.5 au commit `c97e1b2132793cae583fa4819c31cf6465b645f5`. `stmttab`, `syntable`, `exexpr`, `execnl`, `argstack`, `opstack` et `errors` confirment une architecture privée fondée sur tables, sous-grammaires, pile d’opérateurs, dispatch indirect et reprise d’erreur centralisée. Ces constats sont maintenant explicitement documentés comme critères de revue du portage RV.
- 2026-08-03 — Le lecteur numérique du parseur tokenisé isolé accepte maintenant une partie fractionnaire décimale, avec conversion target-side par division D et sans conversion hôte. Le test QEMU passe sur `2.5*(3+4)` et vérifie le motif exact de `17.5`.
- 2026-08-03 — Le payload assembleur principal a été organisé en six modules ordonnés (`bootstrap`, REPL/dispatch, expressions, tableaux/fonctions, chaînes/tables, session), avec un composeur et un contrôle `cmp` contre le miroir `payload-repl.rv`. `build.rs` surveille maintenant chaque module ; composition, génération du payload et fixture QEMU modulaire passent. Le `cargo check` du binaire guest reste dépendant de sa cible RISC-V et ne doit pas être interprété comme une régression host.
- 2026-08-03 — J’ai réessayé le raccordement du parseur tokenisé isolé à `eval_expression` dans le payload complet. La preuve QEMU a montré une régression des commandes directes et de `basic` : le buffer source était correct à l’entrée, mais le nouveau chemin ne respectait pas encore les contrats de registres et de reprise du parseur historique. J’ai retiré uniquement le branchement actif, conservé le code isolé et le lexer vérifiable, puis confirmé sous QEMU le composeur, le parseur tokenisé, le REPL direct, `basic` et le scénario deux lignes. La prochaine tentative devra d’abord publier un contrat d’ABI minimal et testable, avant toute activation dans le dispatch principal.
- 2026-08-03 — J’ai réactivé l’intégration sur le sous-ensemble numérique et corrigé trois contrats réels par observations QEMU : `x31` devait être restauré après la continuation interne, `x9` devait être sauvegardé avant le lexer (le lexer l’utilise comme offset), et le fallback devait adresser les cellules globales depuis la base `0x82062000` plutôt que depuis la base des tokens. Le payload produit maintenant `5.000000` et `14.000000` par le parseur tokenisé intégré, tandis que `X+Y` reprend sans corruption le parseur historique. Direct, précédence, payload principal, littéral et alias `?` passent sous QEMU ; les cellules `x18/x31/x9` sont documentées dans `MEMORY_MAP.md`.
- 2026-08-03 — La première validation rejetait à tort `)` après un nombre : l’état `x30` avait été testé dans le mauvais sens. Après correction, le payload exécute réellement `2*(3+4)` par réduction de la parenthèse puis de `*`, et la séance complète `2+3*4`, `2*(3+4)`, `END` confirme deux sorties `14.000000`, `END`, sept tokens dans le dernier flux et le marqueur `0x82062720=1`.
- 2026-08-03 — Une tentative d’ajouter les signes unaires au chemin tokenisé intégré a été retirée après vérification. Le premier essai réutilisait `x17`, déjà employé par le lecteur de nombres pour l’état fractionnaire ; après correction locale, l’assemblage interactif a aussi révélé deux labels dépassant la capacité de symboles puis un défaut d’exécution du fixture complet. Aucun code non prouvé n’est conservé : l’état poussé reste le sous-ensemble tokenisé validé pour nombres, opérateurs et parenthèses ; la reprise devra fournir un test d’ABI plus ciblé avant activation unaire.
- 2026-08-03 — Reprise réussie de l’activation unaire : `x24` porte l’état « opérande attendu », la cellule `0x82062740` porte le signe différé, et la profondeur de pile `x10` est sauvegardée autour de la négation binaire. Un diagnostic temporaire a identifié la corruption à la fermeture de parenthèse ; après retrait du diagnostic, QEMU valide `(-2.5)+(+3.5)` par le chemin tokenisé actif, résultat `1.0`, marqueur `0x82062720=1`. Les régressions REPL, précédence et lexer restent vertes ; un signe devant un groupe parenthésé repasse explicitement au parseur historique.
- 2026-08-03 — Reprise ciblée de l’activation unaire : `x24` porte désormais l’état « opérande attendu », la cellule `0x82062740` porte le signe différé, et `x10` est sauvegardé autour de la négation binaire car il contient aussi la profondeur de la pile de valeurs. Le diagnostic temporaire a localisé puis confirmé la correction à la fermeture de parenthèse (`token=3`). QEMU valide réellement `(-2.5)+(+3.5)` par le chemin tokenisé actif, avec résultat `1.0`, marqueur `0x82062720=1`, et régressions REPL, précédence et lexer vertes. Les signes devant un groupe parenthésé repassent explicitement par le parseur historique.
